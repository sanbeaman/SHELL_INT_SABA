package com.sanbeaman.shell.loading.display
{
	import com.sanbeaman.shell.loading.display.core.ProgressDisplayLite;
...
internal class CircleDisplay extends Sprite {
	...
	public function CircleDisplay(vars:Object) {
		...
		//this.track.graphics.lineStyle(_config.trackThickness, _config.color, _config.trackAlpha, true, "normal", CapsStyle.NONE);
	/*
		var matr:Matrix = new Matrix();
		matr.createGradientBox(200,200);
		this.track.graphics.lineGradientStyle(GradientType.LINEAR, [0xFF0000, 0x0000FF], [1, 1], [0, 255],matr,SpreadMethod.REPEAT);
	...
	public function set progress(value:Number):void {
		...
		var r:Number=_config.radius;
		
		//var angle:Number = (_progress * Math.PI * 2);
		var ploaded:Number = _progress * 100;
		var g:Graphics = this.progressRing.graphics;
		g.clear();
		g.lineStyle(_config.trackThickness, _config.color, _config.trackAlpha,false,"none",CapsStyle.NONE);
		//g.lineStyle(_config.trackThickness );
		g.moveTo(r,0);
		
		for (var z = 0; z<=ploaded*3.6; z++) {
			g.lineTo((cos(z*d_r)*r), (sin(z*d_r)*r));
		}
